...
//SimplePatternList gPatterns = { gol_loop, sinefield, sinematrix, sinematrix2, sinematrix3 };
SimplePatternList gPatterns = { tetris_loop, sinefield, gol_loop, sinematrix };
...
void loop()
{
  ...
  EVERY_N_MILLISECONDS( 1000/FRAMES_PER_SECOND ) {
    ...
    for( int i = 0; i < NUM_LEDS; i++ ) {
      ...
      int j = 
        (y & 0x01 == 1)
        ? (y * kMatrixWidth) + (kMatrixWidth - 1 - x)
        : i;
      real_leds[j] = leds[i];
    }
    FastLED.show();

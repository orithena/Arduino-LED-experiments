...
#define MEASURE_HIGH_US 100
#define MEASURE_LOW_US 100

#define THRESHOLD_FACTOR 0.9375

#define FOLLOW_SPEED 0.0625
...
void measure_cycle(State *p) {
  ...
  for( int cycle = 0; cycle < MEASURE_CYCLES; cycle++) {
    for( int led = 0; led < NUM_IR_LEDS; led++ ) {
      ...
      int32_t mdelta = max(high_measure - low_measure, 0);
      if( low_measure > 250 && mdelta < 100 ) {
        measures[led][cycle] = p->measure[led];
      } else {
        measures[led][cycle] = max(high_measure - (low_measure / 5), 0);
      }
#ifdef DEBUG
      ...
      Serial.print("\t");
      Serial.print(max(high_measure - low_measure, 0));
      Serial.print("\t");
      Serial.print(max(high_measure - (5 * log(low_measure)), 0));
      Serial.print("\t");
      Serial.print(max(high_measure - (low_measure / 5), 0));
      Serial.print("\t");
#endif
    }
  }
...
//      Serial.print("\t");
  ...
  p->cycle_counter++;
#ifdef DEBUG
  Serial.print(obstructed ? "500" : "100");
  Serial.println();
#endif
}

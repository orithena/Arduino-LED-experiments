...
void setup() {
  ...
  Serial.begin(230400);
  Serial.println("\n\nHello!\n");
  touchAttachInterrupt(T5, touch_handler, 62);
}

volatile uint64_t touch_detected = 0;
volatile uint32_t touch_last = 0;
...
boolean touched() {
  boolean ret = touch_detected > 0;
  if(ret) {
    Serial.printf("Got one at %d\n", touch_detected);
    touch_detected--;
    if( millis() - touch_last > 100 ) {
      touch_detected = 0;
    }
  }
  return ret;

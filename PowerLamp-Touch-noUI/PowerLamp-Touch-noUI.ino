...
// Replace with your network credentials_PULLUP
const char* ssid     = "Lightbar";

bool update_display = true;
bool update_lights = true;
uint32_t override_white_since = 0;
uint32_t override_white_duration = 600000; //ms
...
void loop_display() {
  EVERY_N_MILLISECONDS( 16 ) {
    ...
    if(override_white_since > 0 && (millis() - override_white_since) < override_white_duration) {
      for( int a = 0; a < NUM_LEDS; a++ ) {
        leds[a] = CRGB(255, 255, 255);
      }
      update_display = true;
    } else {
      override_white_since = 0;
    }
...
void loop_touch_control() {
  EVERY_N_MILLISECONDS( 8 ) {
    ...
    #endif
    if( !touching && touch_count >= MIN_TOUCH ) {
      if( override_white_since == 0 ) {
        override_white_since = millis();
      } else if (millis() - override_white_since > 512) {
        override_white_since = 0;
        update_display = true;
      }
      update_lights = true;

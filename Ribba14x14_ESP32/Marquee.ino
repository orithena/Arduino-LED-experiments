...
void DrawTextCol(char *text, int x, int y, CRGB col) {
  for (int i = 0; i < strlen(text); i++) {
    DrawSpriteCol(Char(Font5x7, text[i]), 5, x + i * 6, y, col);
  }
}

void MarqueeCol(char *text, int y, int startms, CRGB col) {
  uint32_t ms = millis();
  int l = strlen(text);
  int x = ((ms-startms)>>7)%((l*6)+kMatrixWidth);
  DrawTextCol(text, kMatrixWidth-x, y, col);
}
...
void Clear() {
  for ( byte y = 0; y < kMatrixHeight; y++) {
    for ( byte x = 0; x < kMatrixWidth; x++) {
      leds(x, y)  = CHSV((16*y)+(47*x), 255, 16);
    }
  }
}

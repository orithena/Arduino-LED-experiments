...
void snakes() {
  ...
  EVERY_N_MILLISECONDS(100) {
    for( int i; i < ARRAY_SIZE(snake); i++ ) {
      ...
      double hue_d = (sin(millis()/8121.0)*gHue) + (sin(millis()/7177.0)*i*(255/ARRAY_SIZE(snake)));
      uint8_t hue = hue_d;
...
void doublesnakes() {
  ...
  EVERY_N_MILLISECONDS(20) {
    for( int i; i < ARRAY_SIZE(snake); i++ ) {
      ...
      double hue_d = (sin(millis()/8121.0)*gHue) + (sin(millis()/7177.0)*i*(255/ARRAY_SIZE(dsnake)));
      uint8_t hue = hue_d;

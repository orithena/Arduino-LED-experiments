...
void loop()
{
  int mic_cur = analogRead(A1);
  int mic_delta = mic_cur - mic_old;
  ...
  else if (ms / 100 <= length) {
    DrawTextOneFrame(ms / 100 % length);
  } 
  else if (ms / 100 <= 2*length) {
    RainbowLoop(ms);
  } 
  else {
    FireLoop();
...
void FireLoop() {
  if( firecount % 4 <= 1 ) {
    for( byte x = 0; x < kMatrixWidth+2; x++ ) {
      buf[bufi][x][kMatrixHeight+2] = (byte)0x32 << random(3);
      buf[bufi][x][kMatrixHeight+3] = (byte)0x32 << random(4);
    }
  }
  if( firecount % random(8) == 0 ) {
    byte d = random(kMatrixWidth-1);
    for( byte x = 1; x <=3; x++) {
      for( byte y = 2; y <= 3; y++)  {
        buf[bufi][d+x][kMatrixHeight+y] = 255;
      }
    }
  }

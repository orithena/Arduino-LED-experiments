...
void SinesLoop(uint32_t ms) {
  ...
  for( int x = 0; x < kMatrixWidth; x++) {
    leds[ XYsafe(x, ((1.2+sin((ms+x)/4.0))/2.4)*kMatrixHeight) ].r = 128+mic_delta*8;
    leds[ XYsafe(x, ((1.2+sin((ms+x+7)/4.0))/2.4)*kMatrixHeight) ].g = mic_mean_far;
    leds[ XYsafe(x, ((1.2+sin((ms+x+15)/4.0))/2.4)*kMatrixHeight) ].b = mic_mean/2;
  }
}

CRGB palette(byte shade) {
  double r = 1-cos((((shade+mic_mean_delta)/255.0)*PI)/2);
...
void FireLoop() {
  ...
  if( mic_mean_delta > 8 ) {
    byte w = min(4, mic_mean_delta/8) + 1;

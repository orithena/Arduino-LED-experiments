...
inline void moderunner() {
  ...
  case FADE:
    ...
    if( scenecounter == 0 ) {
      ...
      (255 * (((usr_submode%2)^1) | (scenecounter%2))),
      cycles
        );
      scenecounter %= 6;
    }
  ...
  case RANDOM:
    if( scenecounter == 1 ) {
      ...
      random(192)+63,
      random(128)+127,
      1024+random(2048*(usr_submode+1))
        );
    } 
    else {
...
void sethsv(double hue, double sat, double val) { 
  ...
  if (sat == 0.0) { // Acromatic color (gray).
    ...
    base = (uint8_t)bd;
    rem = _mod(hue,60);
    m = (uint8_t)((((double)(val-bd)*rem)/60)+bd);
    n = (uint8_t)((((double)(val-bd)*(60-rem))/60)+bd);
...
void setrgb(uint8_t r, uint8_t g, uint8_t b) {
  setraw(r*curve(r*RED_FACTOR/256.0), g*curve(g*GREEN_FACTOR/256.0), b*curve(b*BLUE_FACTOR/256.0));
}
